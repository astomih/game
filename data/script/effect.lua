    ...
    local object = {
        ...
        setup = function(self)
            ...
            for i = 1, self.max_particles do
                self.worlds[i] = world()
                self.worlds[i].velocity = vector3(0, 0, 0)
        ...
        update = function(self)
            self.texture:fill_color(color(math.random(), math.random(),
                                          math.random(), 1))
            self.drawer:clear()
            ...
            if self.timer > self.start_lifetime then
                ...
                if not self.looping then self.is_playing = false end
            end
            for i = 1, self.max_particles do
                self.worlds[i].position.x =
                    self.worlds[i].position.x + i * delta_time
